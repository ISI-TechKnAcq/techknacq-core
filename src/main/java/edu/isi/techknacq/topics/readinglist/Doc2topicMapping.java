...
public class Doc2topicMapping {
    ...
    public void Readc2d(String filename){
        try {
            ...
            Logger.getLogger(Doc2topicMapping.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    public void Run(String c2dfile, String keyfile){
        ...
        try {
            ...
            HashMap<Integer, Double> wordweight=new HashMap<Integer, Double>(100);
            ...
            for(int i=0;i<pnum;i++){
                ...
                for(int j=0;j<paper2topic[i].size();j++){
                    ...
                    w+=o.getweight();
                    double w2=0.0;
                    for(int k=0;k<this.topicinwords[tindex].size();k++){
                       Indexpair p=(Indexpair)topicinwords[tindex].get(k);
                        int windex=p.getindex();
                        if(wordweight.containsKey(windex)==false){
                            wordweight.put(windex,p.getweight()*o.getweight());
                        }else{
                            double oldvalue=wordweight.get(windex);
                            wordweight.put(windex,p.getweight()*o.getweight()+oldvalue);
                        }
                        w2+=p.getweight();
                        if(w2>0.6)
                            break;
                ...
                for(int j=0;j<orderword.size();j++){
                    ...
                    int windex=p.getindex();
                    out.write("\t"+this.words.get(windex)+":"+p.getweight());
                }
                out.write("\n");
            }
            out.close();
    ...
    public static void main(String []args){
        if(args.length<1){
            System.out.println("Usage: [topic composition for documens file] [topic weighted key file]");
            System.exit(2);
        }
        Doc2topicMapping mytopic=new Doc2topicMapping();
        mytopic.Run(args[0], args[1]);
        //mytopic.Run("C:\\Users\\linhong\\Documents\\linhong-work\\Data\\acl-full\\mallet-200-2gm\\2015-12-09\\mallet-comp.txt", "C:\\Users\\linhong\\Documents\\linhong-work\\Data\\acl-full\\mallet-200-2gm\\2015-12-09\\mallet-15689-weightedkey.txt");
    }
